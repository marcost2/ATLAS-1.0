...
!  memo = memo + rp*phase(iphase)%bins
   ...
   do ic = 1, phase(iphase)%bins
      ...
      rhop(ic) = (phase(iphase)%rhomax - phase(iphase)%rhomin)*(fi - phase(iphase)%fimin) &
                 /(phase(iphase)%fimax - phase(iphase)%fimin) + phase(iphase)%rhomin
      sphe(ic) = (phase(iphase)%sphemax - phase(iphase)%sphemin)*(fi - phase(iphase)%fimin) &
                 /(phase(iphase)%fimax - phase(iphase)%fimin) + phase(iphase)%sphemin
   end do
   !
